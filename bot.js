...
function logger(msg) {
  return `${moment().format('YYYY-MM-DDTHH:MM')} ${msg}`;
}
...
function getRandomDate() {
  ...
  const nextTick = Math.floor(Math.random() * (max - min + 1)) + min;
  console.log(logger(`Next tick will take place at ${moment(nextTick).format('YYYY-MM-DDTHH:mm')}`));

  return nextTick;
}

const client = new Discord.Client();
const morde = require('./champions/morde')(client, getRandomDate, logger);
